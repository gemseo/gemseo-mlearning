...
def test_init(linear_algo, bootstrap, loo, size):
    ...
    assert len(distribution.regressors) == distribution.size
    for algo in distribution.regressors:
...
def test_learn(distribution):
    ...
    """
    assert pytest.approx(distribution.regressor.intercept[0], 0.1) == 2.0 / 3
    assert pytest.approx(distribution.regressor.coefficients[0], 0.1) == 0.0
...
def test_learn_submodels(distribution, model, intercept, coefficient):
    ...
    Sub-models: f0(x) = -1 + 2x, f1(x) = 1, f2(x) = 1 - 2x
    """
    assert intercept == pytest.approx(distribution.regressors[model].intercept[0], 0.1)
    assert coefficient == pytest.approx(
        distribution.regressors[model].coefficients[0], 0.1
    )
...
def test_evaluate_weights(distribution, point):
    ...
    assert distribution.evaluate_weights(array([point])).shape == (3,)


def test_evaluate_weights_with_several_points(distribution):
    """Check the evaluate function with several points at once."""
    weights = distribution.evaluate_weights(array([[0.0, 0.25, 0.5, 0.75, 1.0]]).T)
