...
"""# Impact of the acquisition criterion on quantile estimation"""
...
plt.plot(history_1[0], concatenate(history_1[1]), marker="o", label="U criterion")
plt.plot(history_2[0], concatenate(history_2[1]), marker="o", label="EI criterion")
plt.plot(history_3[0], concatenate(history_3[1]), marker="o", label="EF criterion")
...
dataset = sample_disciplines([discipline], uncertain_space, "y", 1000, "OT_MONTE_CARLO")
reference_quantile = EmpiricalStatistics(dataset, ["y"]).compute_quantile(level)
print(reference_quantile, active_learning_1.qoi)
print(reference_quantile, active_learning_2.qoi)
print(reference_quantile, active_learning_3.qoi)
...
observations = sample_disciplines(
    [discipline], uncertain_space, "y", n_test * n_test, "OT_FULLFACT"
).values
